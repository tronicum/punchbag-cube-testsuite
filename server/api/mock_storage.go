...
// LoadMockDataFromFile loads mock data from a local file
func LoadMockDataFromFile(c *gin.Context) {
	key := c.Param("key")
	filePath := "mock_" + key + ".json"
	...
	mockDataStore[key] = data
	mockDataMutex.Unlock()
	c.JSON(http.StatusOK, gin.H{"status": "loaded from file", "key": key})
}
