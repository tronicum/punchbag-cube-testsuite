...
// Added new endpoints for Hetzner and IONOS
func (h *ProviderSimulationHandlers) GetProviderInfo(c *gin.Context) {
	provider := c.Param("provider")
	
	// Simulate provider info retrieval
	switch provider {
	case "azure":
		c.JSON(http.StatusOK, gin.H{
			"provider": "azure",
			"info": map[string]interface{}{
				"regions": []string{"eastus", "westus2", "westeurope", "northeurope"},
				"services": []string{"aks", "app-service"},
			},
		})
	case "hetzner-hcloud":
		c.JSON(http.StatusOK, gin.H{
			"provider": "hetzner-hcloud",
			"info": map[string]interface{}{
				"regions": []string{"ash", "fsn1", "hel1", "nbg1", "hil"},
				"services": []string{"kubernetes", "load-balancer"},
			},
		})
	case "united-ionos":
		c.JSON(http.StatusOK, gin.H{
			"provider": "united-ionos",
			"info": map[string]interface{}{
				"regions": []string{"de/fra", "de/txl", "us/las", "us/ewr"},
				"services": []string{"kubernetes", "compute"},
			},
		})
	case "aws":
		c.JSON(http.StatusOK, gin.H{
	...
	case "gcp":
		c.JSON(http.StatusOK, gin.H{
			...
			"info": map[string]interface{}{
				...
				"services": []string{"gke", "compute-engine"},
			},
		})
	default:
		c.JSON(http.StatusBadRequest, gin.H{
