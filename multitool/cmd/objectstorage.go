...
var createBucketCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Fprintf(os.Stderr, "[MT DEBUG] (create) automationMode=%v debugMode=%v provider=%s mode=%s bucket=%#v\n", automationMode, debugMode, provider, mode, bucket)
		if debugMode {
			fmt.Printf("[MT DEBUG] createBucket: provider=%s, mode=%s, bucket=%#v\n", provider, mode, bucket)
		}
		// Use --server (proxyServer) as the endpoint for simulation requests if set
		...
		// In simulate mode, use proxyServer (set by --server) as the endpoint
...
func init() {
	...
	objectStorageCmd.PersistentFlags().BoolVar(&debugMode, "debug", false, "Enable debug output for objectstorage commands")
