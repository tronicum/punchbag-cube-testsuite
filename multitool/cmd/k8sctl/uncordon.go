...
//
// k8sctl uncordon: kubectl-like uncordon command for multitool
package k8sctl
...
var uncordonCmd = &cobra.Command{
	...
	Short: "Mark node as schedulable (kubectl-like)",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Printf("Error running kubectl uncordon: %v\n%s\n", err, string(out))
			return
		}
...
func init() {
uncordonCmd.Flags().String("provider", "", "Cloud provider (hetzner|azure|...)")
uncordonCmd.Flags().String("kubeconfig", "", "Path to kubeconfig file")
