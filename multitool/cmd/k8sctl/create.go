...
//
// k8sctl create: kubectl-like create command for multitool
package k8sctl
...
var createCmd = &cobra.Command{
	...
	Short: "Create a resource from a file or from stdin (kubectl-like)",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Printf("Error running kubectl create: %v\n%s\n", err, string(out))
			return
		}
...
func init() {
	createCmd.Flags().String("provider", "", "Cloud provider (hetzner|azure|...)")
	createCmd.Flags().String("kubeconfig", "", "Path to kubeconfig file")
