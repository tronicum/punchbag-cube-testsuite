...
//
// k8sctl cluster-info: kubectl-like cluster-info command for multitool
package k8sctl
...
var clusterInfoCmd = &cobra.Command{
	Use:   "cluster-info [flags]",
	Short: "Display cluster information (kubectl-like)",
	Args:  cobra.NoArgs,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Printf("Error running kubectl cluster-info: %v\n%s\n", err, string(out))
			return
		}
...
func init() {
	clusterInfoCmd.Flags().String("provider", "", "Cloud provider (hetzner|azure|...)")
	clusterInfoCmd.Flags().String("kubeconfig", "", "Path to kubeconfig file")
