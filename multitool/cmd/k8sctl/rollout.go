...
//
// k8sctl rollout: kubectl-like rollout command for multitool
package k8sctl
...
var rolloutCmd = &cobra.Command{
	Use:   "rollout [subcommand] [flags]",
	Short: "Manage the rollout of a resource (kubectl-like)",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Printf("Error running kubectl rollout: %v\n%s\n", err, string(out))
			return
		}
...
func init() {
	rolloutCmd.Flags().String("provider", "", "Cloud provider (hetzner|azure|...)")
	rolloutCmd.Flags().String("kubeconfig", "", "Path to kubeconfig file")
