...
//
// k8sctl drain: kubectl-like drain command for multitool
package k8sctl
...
var drainCmd = &cobra.Command{
	...
	Short: "Drain node in preparation for maintenance (kubectl-like)",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Printf("Error running kubectl drain: %v\n%s\n", err, string(out))
			return
		}
...
func init() {
drainCmd.Flags().String("provider", "", "Cloud provider (hetzner|azure|...)")
drainCmd.Flags().String("kubeconfig", "", "Path to kubeconfig file")
