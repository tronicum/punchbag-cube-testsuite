...
//
// k8sctl top: kubectl-like top command for multitool
package k8sctl
...
var topCmd = &cobra.Command{
	Use:   "top [resource] [flags]",
	Short: "Display resource (CPU/memory) usage (kubectl-like)",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Printf("Error running kubectl top: %v\n%s\n", err, string(out))
			return
		}
...
func init() {
topCmd.Flags().String("provider", "", "Cloud provider (hetzner|azure|...)")
topCmd.Flags().String("kubeconfig", "", "Path to kubeconfig file")
